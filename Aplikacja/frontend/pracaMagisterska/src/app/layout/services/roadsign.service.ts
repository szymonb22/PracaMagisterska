...
import { DetectedSign, PagedSign, RoadSign } from '../models/roadsign.model';
@Injectable({
  providedIn: 'root'
})
export class RoadsignService {
  ...
  signRecognize(file:any){
    return this.http.post(this.ApiUrl+'recognize',file)
  }

  changeModel(nrModel){
    return this.http.post(this.ApiUrl+'changeModel',nrModel)
  }
}
